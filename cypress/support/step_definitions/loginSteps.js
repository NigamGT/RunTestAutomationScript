...
cy.get('input[name="loadFactor"]',{timeout:50000}).clear({force:true}).type('100',{force:true});
cy.get('input[name="loadFactor"]',{timeout:50000}).clear({force:true}).type('100',{delay: 100 ,force:true});
...
.clear({force:true}).type("Doppel"+username, { delay: 100 ,force: true});
cy.selectFirstOptionFromDropDown();
cy.get('button[type="submit"]').click({force:true});
